function McQueen(x, altitude, enemy, size, color) {
  ...
  this.enemy = enemy; // enemy or not
	...
	this.drone = 0; // enemy Mcqueen drone across the screen
 ...
 * moves enemy Mcqueen across the screen
 */
McQueen.prototype.update = function() {
 ...
 */
McQueen.prototype.jump = function() {
 ...
 */
McQueen.prototype.applyForce = function(force) {

  this.velocity.add(force);
};

/**
 * returns whether or not the McQueen collides with another McQueen
 */
McQueen.prototype.collidesWith = function(McQueen) {

  var distance = dist(McQueen.location.x, McQueen.location.y, this.location.x, this.location.y);

  if (distance < (this.size / 2 + McQueen.size / 2)) {
		// distance is greater than radii combined

    if (McQueen.location.y < this.location.y) {
			// underneath McQueen
...
/**
 * draws the McQueen with specific altitiude translation
 */
McQueen.prototype.draw = function(altitude) {
